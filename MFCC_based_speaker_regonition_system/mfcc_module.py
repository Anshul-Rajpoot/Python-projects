    ...
    class MFCCProcessor:
        def __init__(self, sr=16000, frame_size=25, frame_stride=10, n_fft=512, n_mels=20, n_mfcc=13, pre_emphasis=0.97):
            """
        ...
        def compute_mfcc_coefficients(self, log_mel_energy):
            ...
            """
            # Note: The problem statement says "exclude the first component, c0" [cite: 125]
            # Common practice for N_MFCC coefficients is to take 0 to N_MFCC-1.
            # If the intent is truly to exclude c0 and get, say, 12 MFCCs, then n_mfcc should be 12
            # and the slice would be [:, 1:self.n_mfcc+1].
            # However, your original app.py was doing [:, :n_mfcc] which *includes* c0 if n_mfcc is 13.
            # I will keep consistency with your app.py for now, which takes n_mfcc coefficients
            # starting from c0.
        ...
        def plot_spectrogram(signal, sr, title="Spectrogram"):
            ...
            img = librosa.display.specshow(D, sr=sr, x_axis='time', y_axis='mel', ax=ax)
